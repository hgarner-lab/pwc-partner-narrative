...
function humaniseCopy(card) {
  ...
  if (!pre) return;
  const proof = text(card.querySelector(".source-proof span")?.textContent);
  pre.textContent = `The way I would put it is this:\n\nI am seeing clients move past the question of whether AI can create more output. The harder question is where human judgement still needs to shape what happens next.\n\nThe proof I would use: ${proof}\n\nThat feels like the conversation worth having.`;
...
document.addEventListener("change", (event) => {
  if (["sector-select", "role-select", "moment-select", "output-select", "style-select"].includes(event.target.id)) setTimeout(storeSelection, 0);
}, true);
...
document.addEventListener("click", async (event) => {
  ...
  if (copyKitButton) {
    ...
    const copy = [...document.querySelectorAll(".kit-card")].map((card) => `${text(card.querySelector(".card-label")?.textContent)}\n${card.querySelector("pre")?.textContent || ""}`).join("\n\n");
    await copyText(copy);
